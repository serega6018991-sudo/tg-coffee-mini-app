...
check('Home explains the primary coffee flow first without stacking heavy cards', /function BeginnerHome/.test(app) && /Как это работает/.test(app) && /Закажи кофе и забери без очереди/.test(app) && /Выбери кофе/.test(app) && /Оплати и назови код/.test(app) && /simple-steps-card[\s\S]*background: transparent/.test(styles) && /simple-steps-card[\s\S]*box-shadow: none/.test(styles));
check('Secondary home features are compact rows, not feature tiles', /plain-feature-list[\s\S]*grid-template-columns: minmax\(0, 1fr\)/.test(styles) && /plain-feature-list button[\s\S]*background: transparent/.test(styles) && /plain-feature-list button[\s\S]*box-shadow: none/.test(styles) && /plain-feature-list button[\s\S]*min-height: 58px/.test(styles) && /Баланс/.test(app) && /Код выдачи/.test(app) && !/Пополняешь на 1000 грн/.test(app));
